...
const emit = defineEmits(['update:params', 'changePage'])
...
<template>
  <div>
    <div class="flex justify-end flex-wrap my-3">
      <InputLabel
        value="rows per page"
        class="px-3 py-2 mb-1 mr-1 text-xs leading-4 text-gray-400 font-normal"
      />
      <Select
        @change="change"
      ...
      <template v-for="(link, key) in links">
        <div
          v-if="link.url === null"
          :key="key"
          class="px-3 py-2 mb-1 mr-1 text-xs leading-4 text-gray-400 border rounded"
          v-html="link.label"
        />
        <div v-else class="mb-1 mr-1">
          <div
            v-if="link.active"
            :key="key"
            class="px-3 py-2 text-xs leading-4 border rounded bg-blue-50 text-gray-400 border-blue-100"
            v-html="link.label"
          />
          <div
            v-else
            class="flex items-center px-3 py-2 text-xs leading-4 border rounded hover:bg-white focus:border-indigo-500 focus:text-indigo-500"
