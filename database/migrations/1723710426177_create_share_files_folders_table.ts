...
export default class extends BaseSchema {
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.integer('shared_with_user_id').unsigned().references('id').inTable('users').onDelete('CASCADE')
      table.integer('folder_id').unsigned().nullable().references('id').inTable('folders').onDelete('CASCADE')
      table.integer('file_id').unsigned().nullable().references('id').inTable('uploads').onDelete('CASCADE')
